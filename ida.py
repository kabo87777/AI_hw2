...
class puzzle:
    ...
    def manhattan(self):#計算Manhattan distance
        finish = []
        dis = 0
        for i in range(n):
            for j in range(m):
                if self.state[i][j] not in finish:
                    if  int(self.state[i][j])!=0:
                        finish.append(self.state[i][j])
                        dis+=abs(i-final_state_row[self.state[i][j]][0])+abs(j-final_state_row[self.state[i][j]][1])
        return dis

    def IDAs(self):#進行IDA* search
        global nextf
        global final
        flimit = self.manhattan()
        while final==False:
    ...
    def search(self,flimit):#使用flimit做搜尋
        global final
        global IDAans
        global nextf
        if(final):
            ...
            for j in range(4):
                if self.can_move(i,j,1):
                    if i != self.pnum or abs(j-self.pdir)!=2:
                        ...
                        m = next.manhattan()
                        if(self.is_final_state() and self.manhattan()<=flimit):
                            final = True
                            return True
                        if next.depth+m>flimit:
                            if next.depth+m<nextf:
                                nextf = next.depth+m
                            return False
                        else:
                            if(next.search(flimit)):
                                IDAans.append([i,j])            
        return final
...
f_out.write('An optional solution has '+str(len(IDAans))+' moves :\n')
for step in IDAans:
